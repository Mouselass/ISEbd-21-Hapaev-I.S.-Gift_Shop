...
namespace GiftShopClientApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            ViewBag.Gifts = APIClient.GetRequest<List<GiftViewModel>>("api/main/getgiftlist");
            return View();
        }

        [HttpPost]
        public void Create(int gift, int count, decimal sum)
        {
            if (count == 0 || sum == 0)
            {
                return;
